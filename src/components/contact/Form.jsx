...
const commonClass = "input input-lg border-0 border-b-2 focus:outline-none focus:placeholder:text-picto-primary placeholder:text-[15px] md:placeholder:text-lg focus:border-picto-primary border-[#E6E8EB] w-full rounded-none px-0";
...
const Form = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // ✅ Use your actual Formspree form ID
      const response = await fetch('https://formspree.io/f/myznzqnb', {
        method: 'POST',
        body: formData,
        headers: {
  ...
  return (
    <div>
      <p className="text-[12px] xs:text-[14px] max-lg:text-center sm:text-lg font-normal text-soft-dark">
        I'm always open to work opportunities and freelance projects.
      ...
      <div className="mx-2">
        <form className="flex flex-col gap-4 mt-4" onSubmit={handleSubmit}>
          ...
          {status && status !== 'Sending...' && (
            <p className={`text-sm mt-2 ${status.includes('Thanks') ? 'text-green-600' : 'text-red-600'}`}>
              {status}
            </p>
          )}
        </form>
